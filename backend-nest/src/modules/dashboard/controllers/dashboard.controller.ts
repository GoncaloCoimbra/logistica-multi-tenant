...
export class DashboardController {
  ...
  getStats(@Request() req, @Query('companyId') queryCompanyId?: string) {
    ...
    //  SUPER_ADMIN pode filtrar por empresa via query
  ...
  @Roles(Role.ADMIN, Role.OPERATOR, Role.SUPER_ADMIN) //  SUPER_ADMIN adicionado
  @ApiOperation({ summary: 'Produtos por estado' })
  @ApiQuery({ name: 'companyId', required: false, description: 'Filtrar por empresa (SUPER_ADMIN)' })
  getProductsByStatus(@Request() req, @Query('companyId') queryCompanyId?: string) {
    const user = req.user;
  ...
  @Roles(Role.ADMIN, Role.OPERATOR, Role.SUPER_ADMIN) //  SUPER_ADMIN adicionado
  @ApiOperation({ summary: 'Transportes por estado' })
  @ApiQuery({ name: 'companyId', required: false, description: 'Filtrar por empresa (SUPER_ADMIN)' })
  getTransportsByStatus(@Request() req, @Query('companyId') queryCompanyId?: string) {
    const user = req.user;
  ...
  @ApiQuery({ name: 'companyId', required: false, description: 'Filtrar por empresa (SUPER_ADMIN)' })
  getRecentActivity(@Request() req, @Query('companyId') queryCompanyId?: string) {
    const user = req.user;
